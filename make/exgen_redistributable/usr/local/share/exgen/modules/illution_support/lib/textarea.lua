textarea = {};
function textarea:new(cols, rows)
    local object = {};
    local daters = {};
...
--	    end
    ...
    function object:cursor_row ()
	return vt_cursor_row(vt);
    end
    function object:cursor_col ()
...
--[[    daters.surface.old_drawterm = function ()
	...
	--else 
	    ...
	    if (keyval == 0) then
		-- non printing keypresses
		vt_keypress(vt, vt_convscan(keycode));
	    else
		-- printing keypresses
		if (keyval ~= 8) and (keyval ~= 13) then
		    -- all except backspace
		    --this_line = object:read_line();
		    --left = string.sub(this_line, 0, object:cursor_col() - 1);
		    --right = string.char(keyval)..string.sub(this_line, object:cursor_col() + 1, string.len(this_line)-(object:cursor_col() + 1));
		    --object.write(right);
		    --object.write(string.rep(string.char(8), string.len(right)));		    
		    o_row = object:cursor_row();
		    o_col = object:cursor_col();

		    v_start_pos, cursor_pos, v_end_pos = object:render_view();
		    echo("V_Start_Pos: "..v_start_pos.." Cursor_Pos: "..cursor_pos.." V_End_Pos: "..v_end_pos.."\n");
		    left = string.sub(object:read_buffer(), 1, v_start_pos);
		    middle = string.sub(object:read_buffer(), v_start_pos+1, v_end_pos);
		    right = string.sub(object:read_buffer(), v_end_pos);
		    m_left = string.sub(middle, 1, cursor_pos-v_start_pos-1);
		    m_right = string.sub(middle, cursor_pos-v_start_pos+0);
		    --echo(m_right.."\n");
		    --echo(m_left.."<!X!>"..m_right.."\n");
		    object:write_buffer(left..m_left..string.char(keyval)..m_right..right);
		    v_start_pos, cursor_pos, v_end_pos = object:render_view();
		    
		    object:move_cursor(o_row, o_col+1);
		elseif keyval == 13 then -- return
		else
		    echo("Backspace\n\n");
		    --[[ // Old Way
		    this_line = object:read_line();
		    ...
		    right = string.char(keyval)..string.sub(this_line, object:cursor_col() + 1, string.len(this_line)-(object:cursor_col() + 1));
		    object.write(right);
		    object:move_cursor(object:cursor_row(), string.len(left));
		    ]]
		    ...
		    middle = string.sub(object:read_buffer(), v_start_pos+1, v_end_pos);
		    right = string.sub(object:read_buffer(), v_end_pos);
		    m_left = string.sub(middle, 1, cursor_pos-v_start_pos-2);
		    m_right = string.sub(middle, cursor_pos-v_start_pos+0);
		    --echo(m_right.."\n");
		    --echo(m_left.."<!X!>"..m_right.."\n");
		    object:write_buffer(left..m_left..m_right..right);
		    v_start_pos, cursor_pos, v_end_pos = object:render_view();
...
--[[	    this_line = self.read_line();
	...
	for pos = 0, string.len(self.buffer), 1 do
	    ...
	    if object:cursor_row() == c_row then
		if object:cursor_col() == c_col then
		    echo("--Cursor Row: "..c_row.." Cursor Col: "..c_col.."\n");
		    cursor_pos = pos;
		end
	    end
	    f = 0;
	...
	return v_start_pos, cursor_pos, v_end_pos;
    end
