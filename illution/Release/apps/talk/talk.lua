...
create_tween = function(start_value, end_value, steps) 
    tween = {};
    tween.start_value = start_value;
    tween.end_value = start_value;
    tween.steps = step;
    tween.value = start_value;
    tween.step_value = (start_value - end_value) / steps;
    tween.step = 0;
    return tween;
end

run_tween = function(tween, step)
    if type(step) == "nil" then
	step = tween.step;
	tween.step = tween.step + 1;
    end
    tween.value = tween.start_value + (tween.step_value * step);
    return tween.value;
end
