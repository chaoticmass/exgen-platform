...
function latex_interface:new()
	...
	echo(download_string(tex.hostname, tex.url, tex.post_fields .."&action=return_page_count&id="..tex.id));
	...
	if (tonumber(page) > 0) then
	    ...
	    if tex.output_format == "gif" then
		tex.images[1].gif = tex.pages[1];
	    elseif tex.output_format == "jpg" then
		tex.images[1].jpg = tex.pages[1];
	    end
	else
	    if (tex.page_count ~= nil) then
	    if (tonumber(tex.page_count) > 0) then
		for i=1,tex.page_count,1 do
		    ...
		    if tex.output_format == "gif" then
			tex.images[i].gif = tex.pages[i];
		    elseif tex.output_format == "jpg" then
			tex.images[i].jpg = tex.pages[i];
		    end
